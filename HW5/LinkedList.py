class LLNode:
    ...
    def add_last(self, item):
        """Recursively traverses through the LinkedList until we reach the tail node, at which point a new node is added to the LinkedList"""
        #BASE CASE: we are at the tail node since the link points to None
        if self.link == None:
            new_node = LLNode(item, None)
            self.link = new_node
        else: #Else, send the current node's link back into the add_last method
            self.link.add_last(item)


    def sum_all(self):
        """Recursively add all items in linked list and return the sum"""
        #BASE CASE: We are at the tail node, so just return the value in the tail node
        if self.link == None:
            return self.data
        else: #Else, return the sum of the data in the current node plus the sum of the data in the proceeding nodes through recursion
            return self.data + self.link.sum_all()
    

    def reverse(self, prev=None):
        """Reverses the LinkedList through recursion"""
        #BASE CASE: We are at the tail node, so update tail's link to prev and return tail
        if self.link == None:
            self.link = prev
            return self
        else: #Else, store the current node's link, update its link to prev, and send the node's old link into the reverse method
            link = self.link
            self.link = prev
            return link.reverse(self)
