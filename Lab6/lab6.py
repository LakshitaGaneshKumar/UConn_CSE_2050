class OrderedList:
    ...
    # Implement O(logn) binary search
    def _bs(self, item, left, right):
        """searches for item using `left` and `right` indices instead of slicing"""
        # base case - item not in list
        if right - left == 0: return False
        if right - left == 1: 
            if self._L[left] == item: return True
            else: return False
        
        median = (right + left) // 2
        
        if item == self._L[median]: return True                                 # base case: found item
        elif item < self._L[median]: return self._bs(item, left, median)        # item is in smaller half
        elif item > self._L[median]: return self._bs(item, median + 1, right)   # item is in bigger half
